...
# Check whether orchestra hooks are already present
if jq -e '.hooks.SubagentStop' "$SETTINGS" >/dev/null 2>&1; then
    ...
    if $DRY_RUN; then
        ...
        # Merge using jq: preserve existing hooks, add orchestra entries
        ...
        jq -s '
            ...
            # Merge PreToolUse arrays (append orchestra entry, avoid duplication)
            ($existing.hooks.PreToolUse // []) as $existing_ptu |
            ($new_hooks.PreToolUse // []) as $new_ptu |
            # Filter out any already-present orchestra entries (idempotent)
...
if [ ! -f "$STATUS_LINE" ]; then
    ...
    if grep -q "ORCHESTRA_BLOCK_START" "$STATUS_LINE" 2>/dev/null; then
        ...
        if $DRY_RUN; then
            ...
            # Insert block just before the final 'echo -e "$status_line"'
            BLOCK="$REPO/status-line/orchestra-block.sh"
            # Strip shebang + header from block, extract just the functional section
            ...
            # Inject before the final echo line
